...
class DocumentLayout:
    ...
    def heightN(self, node, frag):
        if isinstance(self.node, BlockLayout) and self.node.node.attributes["id"] and self.node.node.attributes["id"] == frag:
            debug("brosakdoasjd")
            return self.y
    ...
    def desmile(self, node, frag):
        if isinstance(node, Text):
            ...
        elif isinstance(node, BlockLayout) or isinstance(node, LineLayout) or isinstance(node, TextLayout):
            if not isinstance(node.node, Text): 
                attr = node.node.attributes
                if "id" in attr and attr["id"] == frag:
                    debug("hey")
                    return node.y
...
class TextLayout:
    ...
    def layout(self):
        ...
        self.font = get_font(size, weight, "roman", style)
...
class BlockLayout:
    ...
    def getDimensions(self, node, attribute):
    ...
    def layout(self):
        ...
        self.getDimensions(self.node, "width")
        if (self.node.style.get("width") == "auto"):
            self.width = self.parent.width
        else:
            self.width = self.node.style.get("width")
        ...
        self.getDimensions(self.node, "height")
...
class Chrome:
    def __init__(self, browser):
        ...
        self.font = get_font(20, "normal", "normal", "roman")
    ...
    def middle_click(self, x, y):
        if self.browser.active_tab:
            self.browser.active_tab.click(x,y)
    ...
    def paint(self):
        ...
        cmds.append(DrawRect(self.bookmarks_rect, None, width=1))
...
class Browser:
    ...
    def new_tab(self, url, middle=False):
...
class Tab:
    ...
    def load(self, url):
        ...
        if "!bookmarks" == body:
            body = "<!doctype html>"
            for bookmarked_tab in self.bookmarks:
                body += "<a href=\"{}\">{}</a><br>".format(bookmarked_tab, bookmarked_tab)
        ...
        if self.url.fragment:
            height = self.document.heightN(self.document.node, self.url.fragment)
            if height != -1:
                self.scroll = height
        self.display_list = []
    ...
    def draw_scrollbar(self):
        ...
        y1 = self.scroll / 2
        y2 = self.scroll / 2 + SCROLL_STEP * 2
        
        self.canvas.create_rectangle(WIDTH - 8, y1, WIDTH, y2, fill='blue', width=0)

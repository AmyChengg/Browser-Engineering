...
class Tab:
    ...
    def submit_form(self, elt):
        ...
        for input in inputs:
            ...
            if "checked" in input.attributes:
                value = "on"
            elif input.attributes.get("type", "") == "checkbox":
                continue
            else:
        ...
        # ch8-get-forms exercise
        if "method" in elt.attributes and elt.attributes["method"].casefold() == "post":
    ...
    #ch8-tab exercise
    def tab(self):
        inputs = [node for node in tree_to_list(self.document.node, []) \
                      if isinstance(node, Element) and node.tag == "input" and \
                        "name" in node.attributes]
        current_index = inputs.index(self.focus)
        self.focus = inputs[(current_index + 1) % len(inputs)]
